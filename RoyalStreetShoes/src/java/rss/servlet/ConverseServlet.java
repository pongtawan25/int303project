 ...
 * @author PONGTAWAN
 */
public class ConverseServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Product> pdList = pdCtrl.findProductEntities();
        List<Product> pd = new ArrayList<>();
        for (Product p : pdList) {
            if (p.getProductbrand().equals("CONVERSE")) {
                pd.add(p);
            }
        }
        request.setAttribute("pd", pd);
        getServletContext().getRequestDispatcher("/Converse.jsp").forward(request, response);
    }
